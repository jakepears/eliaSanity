...
const stateMediator = {
  ...
  mediaIsFullscreen: {
    get(stateOwners) {
      ...
      if (!media || !documentElement)
        return false;
      if (!documentElement[import_fullscreen_api.fullscreenApi.element]) {
        if ("webkitDisplayingFullscreen" in media && "webkitPresentationMode" in media) {
          return media.webkitDisplayingFullscreen && media.webkitPresentationMode === import_constants.WebkitPresentationModes.FULLSCREEN;
        }
        return false;
      }
      if (documentElement[import_fullscreen_api.fullscreenApi.element] === fullscreenElement)
        return true;
      if (documentElement[import_fullscreen_api.fullscreenApi.element].localName.includes("-")) {
        let currentRoot = documentElement[import_fullscreen_api.fullscreenApi.element].shadowRoot;
        if (!(import_fullscreen_api.fullscreenApi.element in currentRoot)) {
          return (0, import_element_utils.containsComposedNode)(
            documentElement[import_fullscreen_api.fullscreenApi.element],
            /** @TODO clean up type assumptions (e.g. Node) (CJP) */
            // @ts-ignore
            fullscreenElement
          );
        }
        while (currentRoot == null ? void 0 : currentRoot[import_fullscreen_api.fullscreenApi.element]) {
          if (currentRoot[import_fullscreen_api.fullscreenApi.element] === fullscreenElement)
            return true;
          currentRoot = (_a = currentRoot[import_fullscreen_api.fullscreenApi.element]) == null ? void 0 : _a.shadowRoot;
        }
      }
      return false;
    },
    set(value, stateOwners) {
      ...
      if (!value && (documentElement == null ? void 0 : documentElement[import_fullscreen_api.fullscreenApi.exit])) {
        const maybePromise = (_a = documentElement == null ? void 0 : documentElement[import_fullscreen_api.fullscreenApi.exit]) == null ? void 0 : _a.call(documentElement);
      ...
      if (fullscreenElement == null ? void 0 : fullscreenElement[import_fullscreen_api.fullscreenApi.enter]) {
        const maybePromise = (_b = fullscreenElement[import_fullscreen_api.fullscreenApi.enter]) == null ? void 0 : _b.call(fullscreenElement);
        if (maybePromise instanceof Promise) {
          maybePromise.catch(() => {
          });
        }
